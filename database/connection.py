...
def init_db(engine):
    ...
    """
    # Create each table in a separate connection to avoid transaction issues
    create_companies_table(engine)
    create_branches_table(engine)
    create_employee_roles_table(engine)
    create_messages_table(engine)
    create_employees_table(engine)
    create_tasks_table(engine)
    create_task_assignments_table(engine)
    create_daily_reports_table(engine)
    
    # Initialize default data
    initialize_default_roles(engine)
    set_default_employee_roles(engine)
...
def create_branches_table(engine):
    """Create branches table if it doesn't exist and add self-reference."""
    try:
        with engine.connect() as conn:
    ...
    # Add self-reference constraint in a separate connection
    try:
        with engine.connect() as conn:
        ...
        st.warning(f"Note: Could not add parent branch constraint: {e}")

def create_employee_roles_table(engine):
...
def create_tasks_table(engine):
    ...
    # Add completed_by_id foreign key in a separate connection
    try:
        with engine.connect() as conn:
            # Check if constraint already exists
            result = conn.execute(text('''
        ...
        st.warning(f"Note: Could not add completed_by_id constraint: {e}")

def create_task_assignments_table(engine):
...
def create_daily_reports_table(engine):
...
def initialize_default_roles(engine):
    """Initialize default roles for companies."""
    try:
        with engine.connect() as conn:
            conn.execute(text('''
        ...
        st.warning(f"Could not initialize default roles: {e}")

def set_default_employee_roles(engine):
    """Set default roles for employees without roles."""
    try:
        with engine.connect() as conn:
            conn.execute(text('''
            UPDATE employees e
        ...
        st.warning(f"Could not set default employee roles: {e}")
